...
public class SList {
	 ...
	 *
	 * IMPORTANT: Be sure you use the equals() method, and not the "==" operator, to
	 * compare items.
	 **/
	...
	/**
	 * twin() takes this list and doubles its length by replacing each node with two
	 * consecutive nodes referencing the same item.
	 *
	 * For example, if the input list is [ 3 7 4 2 2 ], the output list is [ 3 3 7 7
	 * 4 4 2 2 2 2 ].
	 *
	 * IMPORTANT: Do not try to make new copies of the items themselves. Just copy
	 * the references to the items.
	 **/
	...
	public void twin() {
		if (size > 0) {
			...
			SListNode node2 = head.next;
			while (node2 != null) {
				node1.next = new SListNode(node1.item, node2);
				node1 = node2;
				node2 = node1.next;
			}
			node1.next = new SListNode(node1.item, node2);
