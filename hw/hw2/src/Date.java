...
class Date {
	...
	public Date(String s) {
		...
		month = Integer.parseInt(arrOfS[0]);
		day = Integer.parseInt(arrOfS[1]);
		year = Integer.parseInt(arrOfS[2]);
	}
	...
	public static boolean isValidDate(int month, int day, int year) {
		if ((1 <= month) && (month <= 12) && (1 <= day) && (day <= daysInMonth(month, year)) && (1 <= year)) {
			return true;
		} else {
			return false;
		}
	 ...
	 * month, day, and year are printed in full as integers; for example, 12/7/2006
	 * or 3/21/407.
	 * 
	 ...
	 *         the nth day of its year. (366 is only used for December 31 in a leap
	 *         year.)
	 */
	 ...
	 * this Date is 12/15/1997 and d is 12/14/1997, the difference is 1. If this
	...
	public int difference(Date d) {
		...
		if (year == d.year) {
			...
			difference += daysInYear(d.year) - d.dayInYear() + dayInYear();
			for (int i = d.year + 1; i < year; i++) {
				difference += daysInYear(i);
			}
		} else {
			difference -= (daysInYear(year) - dayInYear() + d.dayInYear());
