...
public class BinaryTree implements Dictionary {
	...
	private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
		if (node != null) {
			...
			if (comp < 0) {
				result = findHelper(key, node.leftChild);
			} else if (comp > 0) {
				result = findHelper(key, node.rightChild);
			} else {
				result = node;
			}
			return result;
	...
	public Entry remove(Object key) {
		...
		if (node == null) {
			...
			if (node.leftChild == null && node.rightChild == null) {
				if (node.parent == null) {
					root = null;
				} else if (node.parent.leftChild.equals(node)) {
					node.parent.leftChild = null;
					node.parent = null;
				} else if (node.parent.rightChild.equals(node)) {
				...
				if (node.parent == null) {
					...
					if (node.parent.leftChild != null && node.parent.leftChild.equals(node)) {
						...
						node.parent = null;
					} else if (node.parent.leftChild != null && node.parent.rightChild.equals(node)) {
